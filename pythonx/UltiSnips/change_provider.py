...
class LegacyChangeProvider(ChangeProvider):
    ...
    def consume_edits(self, buf, snippet, vstate):
        cstart = snippet.start.line
        cend = snippet.end.line + (len(buf) - vstate.remembered_buffer_length)
        ct = buf[cstart : cend + 1]
        lt = vstate.remembered_buffer
        ...
        if not rv:
            lt_str = "\n".join(lt)
            ct_str = "\n".join(ct)
            es = diff(lt_str, ct_str, initial_line)
        return es
